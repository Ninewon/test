...
public class AirCon {
    ...
    public AirCon(boolean power, int setTemp, boolean cooler, boolean heater, int step) {
        ...
        int month = now.get(Calendar.MONTH);  // 현재가 몇월인지 정보가 배열의 인덱스와 동일하게 넘어옴
        this.isPower = power;
    ...
    public void setAirCon() {
        ...
        // 온도 설정
        System.out.println("온도 설정 : ");
        setSetTemp(sc.nextInt());
        // 바람세기 설정
        System.out.println("바람 세기 설정(1/2/3) : ");
        setWindStep(sc.nextInt());
        // 조건에 따라서 히터와 쿨러 켜고 끄기
        if (getCurrTemp() > getSetTemp()) {   // 현재 온도가 설정 온도 보다 높은 상태
